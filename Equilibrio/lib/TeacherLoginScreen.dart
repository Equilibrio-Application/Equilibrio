...
class LoginAsTeacher extends StatefulWidget {
  @override
  _MyStateTeacher createState() => _MyStateTeacher();
}

class _MyStateTeacher extends State<LoginAsTeacher> {
  ...
  static Future<bool> checkExist(String docID) async {
    bool exists = false;
    try {
      await Firestore.instance.document("Teacher/$docID").get().then((doc) {
        if (doc.exists)
          exists = true;
        else
          exists = false;
